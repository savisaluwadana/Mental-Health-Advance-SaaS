...
import { asMongoMessage, asMongoSession, serverApi } from '@/lib/server-api'
...
export default async function PractitionerDashboardHome() {
  ...
  const [sessionsResult, alertsResult, clientsResult] = await Promise.all([
    serverApi<{ sessions: any[] }>('/sessions'),
    serverApi<{ alerts: any[] }>('/messages/alerts').catch(() => ({ alerts: [] })),
    serverApi<{ clients: any[] }>('/clients').catch(() => ({ clients: [] })),
  ])
  const sessions = sessionsResult.sessions.map(asMongoSession)
  const todaySessions = sessions.filter((s: any) => {
    const scheduledAt = new Date(s.scheduledAt)
    return scheduledAt >= today && scheduledAt < todayEnd && ['pending', 'confirmed'].includes(s.status)
  })
  const weekSessions = sessions.filter((s: any) => {
    const scheduledAt = new Date(s.scheduledAt)
    return scheduledAt >= weekStart && scheduledAt <= weekEnd && ['confirmed', 'completed'].includes(s.status)
  }).length
  const flaggedMessages = alertsResult.alerts
    .map((alert: any) => asMongoMessage({ ...alert.message, sender: alert.client }))
    .slice(0, 5)
