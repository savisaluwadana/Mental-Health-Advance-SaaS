...
import { asMongoDoc } from '@/lib/server-api'
...
export default async function ResourcesPage() {
  ...
  const articleResult = await fetch(`${apiUrl}/articles`, { cache: 'no-store' })
    .then((response) => response.json())
    .catch(() => ({ articles: [] }))
  const dbArticles: any[] = (articleResult.articles ?? []).map(asMongoDoc)
  return (
