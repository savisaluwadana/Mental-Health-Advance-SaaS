...
export default async function ClientDashboardHome() {
  ...
  const [upcomingSessions, todayMood, activeGoals] = await Promise.all([
    SessionModel.find({
      clientId: session!.user.id,
      status: { $in: ['pending', 'confirmed'] },
      scheduledAt: { $gte: new Date() },
    })
      .populate('practitionerId', 'name avatar slmcRegNo role')
      .sort({ scheduledAt: 1 })
      .limit(3)
      .lean(),
    MoodEntry.findOne({
    ...
    Goal.find({ clientId: session!.user.id, completedAt: null }).limit(5).lean(),
  ])
  ...
  return (
    <div className="animate-fade-in space-y-8">
        ...
        <div className="card p-6 border-green-200 bg-green-50 dark:bg-green-900/10 dark:border-green-800">
          <div className="flex items-center gap-3">
            <span className="text-3xl">
              {todayMood.score >= 8 ? '😊' : todayMood.score >= 5 ? '😐' : '😔'}
            </span>
            <div>
            ...
            <Link href="/dashboard/client/mood" className="btn-ghost ml-auto text-sm">
              Update
            </Link>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Upcoming Sessions', value: upcomingSessions.length, icon: '📅', href: '/dashboard/client/schedule' },
          { label: 'Active Goals', value: activeGoals.length, icon: '🎯', href: '/dashboard/client/goals' },
          { label: 'Mood Streak', value: todayMood ? '🔥' : '—', icon: '📊', href: '/dashboard/client/mood' },
          { label: 'Messages', value: '—', icon: '💬', href: '/dashboard/client/messages' },
        ].map((stat) => (
        ...
        {upcomingSessions.length === 0 ? (
          ...
          <div className="space-y-3">
            {upcomingSessions.map((s: any) => (
              <div key={s._id.toString()} className="card p-4 flex items-center gap-4">
